# Device1 STM32CubeMX integration
...
Change these pins in the `Device1HardwareConfig_t` instance to match the real PCB. The driver has no hard-coded GPIO ports.
...
if (!Device1App_Initialize(&cfg)) {
    Error_Handler();
}
```

Do not also create CubeMX tasks with the same purpose. Alternatively, remove the internal `osThreadNew` calls and assign `Device1_RadioTask` / `Device1_ApplicationTask` as CubeMX task entry functions.
...
Override the weak `Device1_Log` function. It can enqueue a diagnostic record for a UART task. Avoid blocking UART transmission from `RadioTask`. The library itself does not depend on `printf`.
